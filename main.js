...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		{
			if (Game.goldenCookie.delay!=0 || Game.goldenCookie.life!=0) Game.Win('Cheated bagels taste awful');
		...
		Game.goldenCookie.click=function()
		{
			if (Game.goldenCookie.life>0)
			{
				...
				else if (choice=='multiply cookies')
				{
					...
					Game.Earn(moni);
					Game.Popup('Lucky! +'+Beautify(moni)+' bagels!');
				}
				else if (choice=='ruin cookies')
				{
					...
					Game.Spend(moni);
					Game.Popup('Ruin! Lost '+Beautify(moni)+' bagels!');
				}
				...
				else if (choice=='chain cookie')
				{
					...
					moni=parseInt(moni);
					Game.Popup('Cookie chain : +'+Beautify(moni)+' bagels!');
		...
		Game.getNewTicker=function()
		{
			...
			if (Game.TickerN%2==0 || Game.cookiesEarned>=10100000000)
			{
				...
				'<q>Moist bagels.</q><sig>grandma</sig>',
				...
				'<q>I can smell your rotten bagels.</q><sig>grandma</sig>'
				]));
			...
			if (list.length==0)
			{
				if (Game.cookiesEarned<5) list.push('You feel like making bagels. But nobody wants to eat your bagels, yes even Uri(p).');
				else if (Game.cookiesEarned<50) list.push('Your first batch goes to the trash. Uri(p) barely touches it.');
				else if (Game.cookiesEarned<100) list.push('Your classmates accepts to try some of your bagels.');
				else if (Game.cookiesEarned<500) list.push('Your bagels are popular in the gifted-o-shpere.');
				else if (Game.cookiesEarned<1000) list.push('People are starting to talk about your bagels.');
				else if (Game.cookiesEarned<3000) list.push('Your bagels are talked about for megameters around.');
				else if (Game.cookiesEarned<6000) list.push('Your bagels are renowned in the whole Te-Aviv district!');
				else if (Game.cookiesEarned<10000) list.push('Your bagels bring all the boys to the yard, a thing which is of couse, impossible.');
				else if (Game.cookiesEarned<20000) list.push('Your bagels now have their own website and domain!');
				else if (Game.cookiesEarned<30000) list.push('Your bagels are worth a lot of money.');
				else if (Game.cookiesEarned<40000) list.push('Your bagels sell very well in distant countries.');
				else if (Game.cookiesEarned<60000) list.push('People come from very far away to get a taste of your bagels.');
				...
				else if (Game.cookiesEarned<10000000) list.push('Elder gods from the whole cosmos have awoken to taste your bagels.');
				else if (Game.cookiesEarned<30000000) list.push('Beings from other dimensions lapse into existence just to get a taste of your bagels.');
				else if (Game.cookiesEarned<100000000) list.push('Your bagels have achieved sentience, and now the world is a totalitarian communist dictatorship.');
				else if (Game.cookiesEarned<300000000) list.push('The universe has now turned into bagel dough, to the molecular level.');
				else if (Game.cookiesEarned<1000000000) list.push('Your bagels are rewriting the fundamental laws of the universe, now you\'ll need to relearn them.');
				else if (Game.cookiesEarned<10000000000) list.push('A local news station runs a 10-minute segment about your bagels. Success!<br><span style="font-size:50%;">(you win a cookie)</span>');
		...
		//define objects
		new Game.Object('Cursor','cursor|cursors|clicked','Autoclicks once every shimshon second, washes its hand once a day, twice-daily and thrice-yearly, a thing which is of course, impossible.','cursor','cursoricon','',15,function(){
			var add=0;
			if (Game.Has('Thousand fingers')) add+=0.1;
			if (Game.Has('Million fingers')) add+=0.5;
			if (Game.Has('Billion fingers')) add+=2;
		...
		new Game.Object('Farm','farm|farms|harvested','Grows bagel plants from bagel seeds, a thing which is of course, impossible.','farm','farmIcon','farmBackground',500,function(){
		...
		new Game.Object('Factory','factory|factories|mass-produced','Produces large quantities of bagels, managed by the barilaners that will be forever loyal to their owner (right?)','factory','factoryIcon','factoryBackground',3000,function(){
		...
		new Game.Object('Mine','mine|mines|mined','Mines out bagels, dough and sweet potato chips, a thing which is of course, impossible.','mine','mineIcon','mineBackground',10000,function(){
		...
		new Game.Object('Shipment','shipment|shipments|shipped','Brings in fresh bagel from the Bagel Planet, the way is through the new universal highway, yes that one.','shipment','shipmentIcon','shipmentBackground',40000,function(){
		...
		new Game.Object('Alchemy lab','alchemy lab|alchemy labs|transmuted','Turns gold into bagels! Fully eatable bagels (right?)','alchemylab','alchemylabIcon','alchemylabBackground',200000,function(){
		...
		new Game.Object('Portal','portal|portals|retrieved','Opens a door to the Bagelverse.','portal','portalIcon','portalBackground',1666666,function(){
		...
		new Game.Object('Time machine','time machine|time machines|recovered','Brings bagels from the past, before they were even eaten, a thing which is of course, impossible.','timemachine','timemachineIcon','timemachineBackground',123456789,function(){
		...
		new Game.Object('Antimatter condenser','antimatter condenser|antimatter condensers|condensed','Condenses the antimatter in the universe into bagels. Fully safe for eating (maybe?)','antimattercondenser','antimattercondenserIcon','antimattercondenserBackground',3999999999,function(){
		...
		*/
		...
		order=30200;
		new Game.Achievement('Cheated bagels taste awful','Hack in some cookies.',[10,6],3);
		order=30001;
	...
	Game.Logic=function()
	{
		...
		if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
		{
			...
			if (Game.cookiesEarned<Game.cookies) Game.Win('Cheated bagels taste awful');
		...
		if (Game.T%(Game.fps)==0) document.title=Beautify(Game.cookies)+' '+(Game.cookies==1?'bagel':'bagels')+' - Bagel Clicker';
